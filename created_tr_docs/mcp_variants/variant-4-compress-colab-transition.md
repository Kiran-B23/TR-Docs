...
In the **Integrating MCP** session, the SkillMap Agent ended up with this line:
...
Every session so far has run in Google Colab. This one cannot: an MCP server using **stdio** is launched as a subprocess by whichever client wants to use it, so it needs a real command on a real machine. For this session we work **locally**.

| In Colab | Locally | Why |
|----------|---------|-----|
| A cell you run | A `.py` file | A client cannot run a cell |
| `!pip install X` | `uv add X` | Dependencies must be installed where `uv run` can find them |
| The runtime resets | A project folder | Your server has to still work tomorrow |

`uv run` guarantees your server starts in the environment you installed into — not whatever shell the client happens to use. That single guarantee is why every launch command in this session is `uv run server.py` rather than `python server.py`. We use `uv` because it is what the official MCP documentation uses.

| Requirement | Why |
|-------------|-----|
| Python 3.10+ | Required by the MCP Python SDK |
| MCP Python SDK 2.0+ | The API changed significantly in 2.x |
| Node.js 22.19+ | For the MCP Inspector testing tool |
...
So: nothing to add to `server.py`. Our server still exposes the three tools, one resource and one prompt we set out to build — which is what you will see in the Final Code below.
