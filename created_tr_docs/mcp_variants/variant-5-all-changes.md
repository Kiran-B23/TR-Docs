...
In the **Integrating MCP** session, getting Tavily's search into the agent took one line — a URL. Getting `search_jobs` into the same agent took copying the entire function definition into the notebook. You already did that work. And it was fine — once.

Now imagine doing it again for the Interview Assistant. And again for your teammate's project. And again next week, after you fix the bug in line 12.

That is the problem, and it is visible in one line:
...
Both tools sat side by side in the same agent and worked equally well — but they are not equal at all:
...
If you wanted to give `search_jobs` to a teammate or to another agent you are building, you would have to send the source code, the API keys, and hope they set it up right. Tavily? Send a URL.
...
Before we build, one question: **what kind of thing** should each of our SkillMap capabilities be?
...
After Step 7 the server is live and connected. The **Going Deeper** sections that follow explain the choices we made — transports, error handling, versioning, and elicitation — without interrupting the build.

---

## Heads Up: We Are Leaving Colab

Every session so far has run in Google Colab. This one cannot: an MCP server using the **stdio** transport is launched *as a subprocess* by whichever client wants to use it, and a Colab notebook is not a command another program on your computer can start. So we work **locally**: a folder, a `server.py` file, and a terminal.

| In Colab | Locally | Why |
|----------|---------|-----|
| A cell you run | A `.py` file a command runs | A client cannot "run a cell" |
| `!pip install X` | `uv add X` | The client must start your server where `X` exists |
| The runtime resets | A project folder that persists | Your server has to still work tomorrow |

`uv run` guarantees the right environment. That matters because a client launches your server, not you. We use `uv` because it is what the official MCP documentation uses.
...
That is a real `tools/list` exchange against your own server. We will use this command properly in Step 6; right now it is a smoke test, and it is worth running every time you add something.
...
You just ran `tools/list` against your own server and got back a schema. Let's look at what actually happened on the wire — because it explains why that twelve-line file was enough.
...
Every connection opens with this. The server you just built answers:
...
That `initialize` exchange is what the SDK you installed actually does, and it is what you saw in the Inspector. But the **specification has already moved past it.**
...
This is the exchange that just ran when you tested your server. The client asks what exists. The server answers with names *and machine-readable schemas*:
...
That is exactly what `get_tools()` did in the Integrating MCP session. It sent `tools/list`, read the schemas that came back, and turned each one into a LangChain tool. Tavily's search "just appeared" because it could be *asked about*.

Our `search_jobs` could not travel for one reason: **nothing could ask it that question.** There was no address to send `tools/list` to. That is not a code-quality problem. It is a missing conversation — and the twelve lines you just wrote are that conversation.
...
`ToolError` is how a tool reports a failure that the **model** should see — the **Structured Error Returns** section under Going Deeper explains why that distinction matters. For now, read it as "fail with a message the model can act on".
...
It is acceptable here for one reason only: on **stdio**, your server is a subprocess launched by one user on one machine, so the operating system is the boundary. Put the same server on a public URL and that boundary disappears — which is what the authentication note in the Transports section under Going Deeper is really about.
...
The server works and the agent consumes it. Now let's understand the choices we made — and the ones we deferred.
...
**Put the key back before moving on** — the versioning section re-runs a real `search_jobs` call, and a missing key there would hide the failure it is meant to show.
...
That is the sentence worth carrying out of this section. Every call that can fail — network, parsing, a missing key — either becomes a `ToolError` with a message, or becomes nothing the model can use.
...
Everything up to this point assumed we are the only person using this server. That assumption expired the moment we connected an agent in Step 7.
...
Now re-run the *same* Inspector call you ran in Step 6 — the one that worked:
...
Elicitation is new enough that client support is still uneven, so the code above may not complete a round trip in whatever client you happen to be using. That is a property of the ecosystem today, not a mistake in the code — and another reason we are reading this section rather than building it.
...
So: nothing to add to `server.py`. Our server still exposes the three tools, one resource and one prompt we set out to build — which is what you will see in the Final Code above.
...
* **The `try`/`except` is already there.** Not as polish, but because an unconverted exception reaches the model as nothing useful (see Structured Error Returns under Going Deeper). Start with it and you never have to remember it — then narrow `Exception` to whatever your work actually raises, as we did with `requests.RequestException`.
...
* **HTTP+SSE as a transport** — replaced by Streamable HTTP, as covered in the Transports section under Going Deeper.
...
| `Error executing tool X` with no reason | An exception escaped that was not a `ToolError` (see Structured Error Returns under Going Deeper) |
...
**3 — This is the Structured Error Returns lesson again, from the other side.** A set is not a `list[dict]`, so output validation fails — and because that failure is not a `ToolError`, the model receives:
...
The line worth carrying forward: building a server is writing the answers to four questions — and once you have, anything that speaks the protocol can use what you built.
